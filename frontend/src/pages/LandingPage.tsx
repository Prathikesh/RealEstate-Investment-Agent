...
function Nav({ scrolled, menuOpen, setMenuOpen }: { scrolled: boolean; menuOpen: boolean; setMenuOpen: (v: boolean) => void }) {
  const links = [{ label: 'Features', href: '#features' }, { label: 'How it works', href: '#how' }, { label: 'FAQ', href: '#faq' }]
  return (
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/90 backdrop-blur-md border-b border-surface-border shadow-sm' : 'bg-transparent'}`}>
      <nav className="max-w-7xl mx-auto px-5 sm:px-8 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-2">
          ...
          <Link to="/register" className="group inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-accent text-white text-sm font-bold shadow-sm shadow-accent/30 hover:bg-accent-hover hover:shadow-md transition-all">Get started <ArrowRight size={15} className="group-hover:translate-x-0.5 transition-transform" /></Link>
        </div>
      ...
      <div className="relative max-w-7xl mx-auto px-5 sm:px-8 pt-28 pb-20 lg:pt-36 lg:pb-28 grid lg:grid-cols-[1.05fr_0.95fr] gap-12 lg:gap-10 items-center">
        <div>
          <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/10 ring-1 ring-white/15 text-xs font-semibold text-blue-100">
            <span className="w-1.5 h-1.5 rounded-full bg-score-strong animate-pulse" /> Quebec real-estate investment intelligence
          </span>
          <h1 className="mt-5 text-[2.6rem] sm:text-6xl font-extrabold tracking-tight leading-[1.05] text-balance">
            Spot the undervalued Quebec property <span className="bg-gradient-to-r from-blue-400 to-blue-600 bg-clip-text text-transparent">before anyone else.</span>
          </h1>
          <p className="mt-6 text-lg text-slate-300 max-w-xl leading-relaxed">
            Plexa continuously scans the Quebec market, scores every listing with AI, reveals hidden{' '}
            <span className="text-white font-semibold">development potential</span> from official records, and alerts you the instant an opportunity appears.
          </p>
          <div className="mt-8 flex flex-wrap items-center gap-3">
            <Link to="/register" className="group inline-flex items-center gap-2 px-6 py-3.5 rounded-xl bg-accent text-white font-bold shadow-lg shadow-accent/30 hover:bg-accent-hover hover:-translate-y-0.5 transition-all">Start free <ArrowRight size={18} className="group-hover:translate-x-0.5 transition-transform" /></Link>
            <a href="#how" className="inline-flex items-center gap-2 px-6 py-3.5 rounded-xl bg-white/10 ring-1 ring-white/15 text-white font-semibold hover:bg-white/15 transition-colors">See how it works</a>
          </div>
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden shadow-2xl ring-1 ring-white/10 rotate-[0.6deg]">
      ...
      <div className="absolute bottom-4 left-4 right-4 flex items-end justify-between gap-3">
        ...
        <div className="flex flex-col items-center rounded-xl bg-white/95 backdrop-blur px-3 py-2 shadow-lg shrink-0"><span className="text-lg font-extrabold text-score-strong leading-none tabular-nums">82</span><span className="text-[9px] font-bold text-score-strong uppercase tracking-wide mt-0.5">Strong buy</span></div>
      </div>
    </div>
  )
}
...
function DashboardMock() {
  return <div className="rounded-2xl bg-white shadow-2xl ring-1 ring-black/5 h-[440px] flex items-center justify-center text-muted">Dashboard preview</div>
}
  ...
  return (
    <Section id="coverage" tone="navy">
      <div className="grid lg:grid-cols-2 gap-12 items-center">
        <Reveal>
          <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-accent/25 ring-1 ring-accent/40 text-xs font-bold text-blue-100 uppercase tracking-wide">The Plexa edge</span>
          ...
          <p className="mt-5 text-slate-300 leading-relaxed">A property listed as a house might sit on land the city already allows you to build several units on. Plexa reads the official zoning code and the government lot record, then estimates what could be built — right on the listing, with the source document one click away.</p>
          <ul className="mt-6 space-y-3">{['An interactive map matches the exact zoning polygon by GPS', 'Real lot size from official records — not the listing', 'Framed honestly: a guide, always “confirm with the city”'].map(t => (<li key={t} className="flex items-start gap-2.5 text-sm text-slate-200"><Check size={16} className="text-score-strong mt-0.5 shrink-0" />{t}</li>))}</ul>
...
// ── Features (advanced, consistent) ──────────────────────────────────────────
function Features() {
  const feats = [
    { Icon: LineChart, title: 'AI deal score, 0–100', desc: 'Every listing graded on cap rate, cash flow, discount vs. comparable sales and risk — so you know in seconds whether it deserves a closer look.' },
    { Icon: Building2, title: 'Development potential', desc: 'See what could be built on any lot from official zoning + lot data — the upside most investors never check, surfaced automatically.', highlight: true },
    { Icon: Map, title: 'Interactive zoning map', desc: 'The property pinned inside its exact zone boundary on a live map, with the official bylaw and source document one click away.' },
    { Icon: Calculator, title: 'Rebuild & profit calculator', desc: 'Model a teardown-and-rebuild with your own construction costs and rents — Plexa computes the numbers and the net upside live.' },
    { Icon: BarChart3, title: 'Comparable-sales value gap', desc: 'We find nearby comparable sales and show exactly how far below (or above) market the asking price really is.' },
    { Icon: Bell, title: 'Instant deal alerts', desc: 'Set your criteria once. The moment a matching property — or a price drop — appears, Plexa pings you by email or WhatsApp.' },
  ]
  const also = [
    { Icon: TrendingUp, label: '5-year cash-flow projection' },
    { Icon: AlertTriangle, label: 'Deal-killer overlays (flood, agricultural)' },
    { Icon: History, label: 'Full price history per listing' },
    { Icon: Landmark, label: 'Live Quebec taxes & welcome tax' },
    { Icon: ShieldCheck, label: 'Risk assessment on every deal' },
    { Icon: MapPin, label: 'Neighbourhood context & benchmarks' },
  ]
  return (
    <Section id="features">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5 mt-14">
        {feats.map((f, i) => (<Reveal key={f.title} delay={(i % 3) * 90}><FeatureCard {...f} /></Reveal>))}
      </div>

      {/* advanced extras strip */}
      <Reveal delay={120}>
        <div className="mt-6 rounded-2xl bg-white border border-surface-border shadow-card p-6 sm:p-7">
          <p className="text-xs font-bold uppercase tracking-widest text-accent mb-4">Also included on every property</p>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4">
            {also.map(a => (
              <div key={a.label} className="flex items-center gap-3">
                <span className="w-9 h-9 rounded-lg bg-accent/10 text-accent flex items-center justify-center shrink-0"><a.Icon size={17} /></span>
...
function FeatureCard({ Icon, title, desc, highlight }: { Icon: any; title: string; desc: string; highlight?: boolean }) {
  return (
    <div className={`group relative h-full rounded-2xl p-7 border transition-all hover:-translate-y-1 hover:shadow-card-hover ${highlight ? 'bg-white border-accent shadow-lg shadow-accent/10 ring-1 ring-accent/20' : 'bg-white border-surface-border shadow-card'}`}>
      {highlight && <span className="absolute top-5 right-5 text-[10px] font-extrabold uppercase tracking-wide text-accent bg-accent/10 px-2 py-1 rounded-full">Biggest edge</span>}
      <span className="inline-flex w-12 h-12 rounded-xl items-center justify-center mb-5 bg-accent text-white shadow-md shadow-accent/25 transition-transform group-hover:scale-105"><Icon size={22} /></span>
...
function ScoreExplainer() {
  ...
  return (
    <Section tone="navy">
      <div className="grid lg:grid-cols-2 gap-12 items-center">
        <Reveal>
          <span className="text-xs font-bold uppercase tracking-widest text-blue-300">The score</span>
          <h2 className="mt-3 text-3xl sm:text-4xl font-extrabold tracking-tight text-white text-balance">One number tells you if it’s worth your time</h2>
          <p className="mt-5 text-slate-300 leading-relaxed">Every property gets a 0–100 deal score that blends cash flow, cap rate, discount to market and risk. Sort your whole market by it and the best opportunities float to the top instantly.</p>
          <div className="mt-6 flex flex-wrap gap-2">
            {bands.map(b => (<span key={b.label} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/10 ring-1 ring-white/15 text-sm font-semibold text-white"><span className="w-2.5 h-2.5 rounded-full" style={{ background: b.color }} /> {b.label} <span className="text-slate-400 tabular-nums">{b.range}</span></span>))}
          </div>
        </Reveal>
        <Reveal delay={120}>
          <div className="rounded-2xl bg-white p-7 shadow-2xl ring-1 ring-black/5">
            ...
            <div className="h-4 rounded-full overflow-hidden flex">{bands.map(b => (<div key={b.label} className="flex-1" style={{ background: b.color }} />))}</div>
            ...
            <div className="mt-6 grid grid-cols-3 gap-2.5"><MockStat label="Cap rate" value="4.8%" /><MockStat label="Cash flow" value="+$310" tone="text-score-strong" /><MockStat label="vs. market" value="-11%" tone="text-score-strong" /></div>
          </div>
        </Reveal>
      </div>
    </Section>
...
function Testimonials() {
  const quotes = [
    { q: 'I found a triplex 12% under market the morning it was listed. Plexa flagged the development upside I would have completely missed.', n: 'Marc-André L.', r: 'Plex investor · Montréal', img: 'https://randomuser.me/api/portraits/men/32.jpg' },
    { q: 'The zoning read alone is worth it. Knowing a lot can take four units before I even call the broker changes how I bid.', n: 'Sophie T.', r: 'Real-estate investor · Laval', img: 'https://randomuser.me/api/portraits/women/44.jpg' },
    { q: 'I used to spend my evenings refreshing listings. Now the good deals just land in my inbox, already analyzed.', n: 'David R.', r: 'Buy-and-hold investor', img: 'https://randomuser.me/api/portraits/men/75.jpg' },
  ]
  return (
    <Section>
          ...
          <Reveal key={t.n} delay={i * 90}>
            <figure className="h-full rounded-2xl bg-white border border-surface-border shadow-card p-7 flex flex-col">
              ...
              <figcaption className="mt-5 flex items-center gap-3">
                <img src={t.img} alt={t.n} loading="lazy" className="w-11 h-11 rounded-full object-cover ring-2 ring-surface-border"
...
function FAQ() {
  const faqs = [
    ...
    { q: 'What makes the development-potential feature different?', a: 'Most tools stop at the current listing. Plexa matches the property to its exact zoning polygon and reads the official lot record, then estimates how many units could be built — the kind of upside analysis that normally takes a broker or an urban planner.' },
    ...
    <section className="px-5 sm:px-8 py-16 bg-surface">
      <div className="relative max-w-6xl mx-auto rounded-3xl overflow-hidden text-white text-center px-6 py-16 sm:py-20" style={{ background: NAVY }}>
        ...
        <div className="relative">
          ...
          <p className="mt-5 text-lg text-slate-300 max-w-xl mx-auto">Join Quebec investors who find better deals in less time with Plexa.</p>
...
// Sections default to a light-gray ground (never plain white); cards sit white
// on top for contrast. Navy sections add depth and colour rhythm.
